...
if (room = r_firing_range){
	...
	scr_console_add_message(string_build("Left click command received from connection{}", _owner))
	gnet_rpc_send(_owner, scr_cb_client_left_mouse_pressed, _mouse_x, _mouse_y, local_client.m_username)
	//test function
	...
	if (instance_exists(spawn_button)){
		...
		gnet_rpc_send(_owner, scr_cb_create_ship_firing_range, _ship_name, _owner)
	}
